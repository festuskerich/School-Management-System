...
class Book_model extends MY_Model {
    ...
    public function display_book($id = null)
	{
	    $query = $this->db->select('*')->from('books')
	    ->where('isbn_no',$id)->get();
        return $query->result();
	}
    ...
    function saveBookIsbnBarcode($data)
	{
	...
	function saveInventoryBarcode($data)
	{
        $this->db->insert('barcodes',$data);
        $this->db->trans_complete();
        if ($this->db->trans_status() === false) {
	...
	public function getisbn() {
	    $query=$this->db->query("select * from barcodes where type = 'ISBN'");
		return $query->result();
    }
    
    public function getinventorybarcode() {
	    $query=$this->db->query("select * from barcodes where type = 'INVENTORY'");
		return $query->result();
    }
    
    function deleteisbn($id)
	{

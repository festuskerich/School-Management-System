...
class Isbn extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $data['title']      = 'Add Book ISBN';
        $data['title_list'] = 'Book Details';
        $listbook           = $this->book_model->listbook();
        $data['listbook']   = $listbook;
        $this->load->view('layout/header');
        $this->load->view('admin/book/createisbn', $data);
    ...
    public function getBookById()
    {
        ...
        if ($book_isbn != "") {
            $data = $this->book_model->display_book($book_isbn);
    		foreach($data as $row)
    		{
    			  $data = $row->id.",".$row->book_title.",".$row->qty;
    		}
        }
        echo $data;
    }
    ...
    public function saveBookIsbnBarcode() {
        date_default_timezone_set("Africa/Nairobi");
        $data['type']     = 'ISBN';
        $data['dat']      = date('Y-m-d h:i:s');
		$data['barcode']  = $this->input->post('book_isbn');
		$data['description']  = $this->input->post('isbn_description');
		$response=$this->book_model->saveBookIsbnBarcode($data);
		if($response == true) {
            echo "success";
		}
		else {
    ...
    public function saveInventoryBarcode() {
        date_default_timezone_set("Africa/Nairobi");
        $data['type']     = 'INVENTORY';
        $data['dat']      = date('Y-m-d h:i:s');
		$data['barcode']  = $this->input->post('item_barcode');
		$data['description']  = $this->input->post('barcode_description');
		$response=$this->book_model->saveInventoryBarcode($data);
		if($response == true) {
            echo "success";
		}
		else {

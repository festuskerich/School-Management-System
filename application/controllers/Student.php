...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($insert) {
                ...
                $sender_details = array('student_id' => $insert_id, 'admission_no' => $admission_no, 'contact_no' => $this->input->post('guardian_phone'), 'email' => $this->input->post('guardian_email'));
    ...
    public function edit($id)
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            } else if (!isset($siblings_counts) && $sibling_id == 0 && $total_siblings > 4) {
                // add for new parent
                $parent_password = $this->role->get_random_password($chars_min = 6, $chars_max = 6, $use_upper_case = false, $include_numbers = true, $include_special_chars = false);
